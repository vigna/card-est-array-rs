...
/// Benchmarks `estimate` for HyperLogLog with 5-bit registers (1B elements),
/// 6-bit registers (10B elements), and HyperLogLog8 (byte registers),
/// comparing LogLog-β correction (BETA=true) vs classic HLL (BETA=false).
fn bench_estimate(c: &mut Criterion) {
    ...
    for &log2_regs in &[6, 8, 10, 12] {
        ...
        // HyperLogLog with 5-bit registers (1B elements)
        {
            ...
            group.bench_function(BenchmarkId::new("hll-5bit/beta", num_regs), |b| {
        ...
        // HyperLogLog with 6-bit registers (10B elements)
        {
            let logic_beta = HyperLogLogBuilder::new(10_000_000_000)
                .log2_num_regs(log2_regs)
                .build::<usize>()
                .unwrap();
            let mut backend_beta = vec![0usize; logic_beta.backend_len()];
            ...
            let logic_classic = HyperLogLogBuilder::new(10_000_000_000)
                .log2_num_regs(log2_regs)
                .beta::<false>()
                .build::<usize>()
                .unwrap();
            ...
            group.bench_function(BenchmarkId::new("hll-6bit/beta", num_regs), |b| {
                b.iter(|| black_box(logic_beta.estimate(black_box(backend_beta.as_slice()))));
            });
        ...
        // HyperLogLog8 (byte registers)
        {
            let logic_beta = HyperLogLog8Builder::new()
                .log2_num_regs(log2_regs)
                .build::<usize>();
            let mut backend_beta = vec![0u8; logic_beta.backend_len()];
            for i in 0..1_000_000usize {
                logic_beta.add(&mut backend_beta, i);
            }

            let logic_classic = HyperLogLog8Builder::new()
                .log2_num_regs(log2_regs)
                .beta::<false>()
                .build::<usize>();
            let mut backend_classic = vec![0u8; logic_classic.backend_len()];
            for i in 0..1_000_000usize {
                logic_classic.add(&mut backend_classic, i);
            }

            group.bench_function(BenchmarkId::new("hll-byte/beta", num_regs), |b| {
                b.iter(|| black_box(logic_beta.estimate(black_box(backend_beta.as_slice()))));
            });
